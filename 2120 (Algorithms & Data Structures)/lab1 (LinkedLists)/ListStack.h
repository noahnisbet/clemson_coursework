...
void ListStack<T>::push(T value){
  // FIFO method, so insert at the start and remove only from the start.
...
template <class T>
T ListStack<T>::pop(){
  // FIFO method, so insert at te start and remove only from the start.
  T start = stack.getFirst();
  stack.removeStart();
  return start;
}
